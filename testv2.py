  ...
  try:
    ...
    if 'forecast' in weather_data and weather_data.get('forecast'):
      for day in weather_data['forecast'].get('forecastday', []):
        ...
        # Calculate HCI for this forecast day
        forecast_hci = None
        if avgt is not None:
          try:
            # Use forecast temp + current humidity (since future humidity may not be reliable)
            forecast_hci = (float(avgt) + float(day_humidity)) / 4.0
            
            # Clamp forecast HCI to stay within current day's range ±10
            if hci is not None:
              hci_float = float(hci)
              min_hci = max(0, hci_float - 10)
              max_hci = min(100, hci_float + 10)
